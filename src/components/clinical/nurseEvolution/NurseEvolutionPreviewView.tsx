...
import { buildNurseEvolutionNote } from '../../../engine/nurseEvolutionNoteBuilder';
import { buildAuthorizedNurseEvolutionFacts } from '../../../engine/nurseEvolutionClinicalFactBuilder';
import { refineNurseEvolutionNoteWithAI } from '../../../services/nurseEvolutionAiRefinementService';
import { AIRefinementRequestResult } from '../../../services/aiRefinementService';
import { checkPrivacyGuards } from '../../../engine/privacyGuard';

interface NurseEvolutionPreviewViewProps {
  form: NurseEvolutionForm;
  onBackToEdit: () => void;
  patientAreaName?: string;
}

export const NurseEvolutionPreviewView: React.FC<NurseEvolutionPreviewViewProps> = ({
  form,
  onBackToEdit,
  patientAreaName = 'PS / Emergência',
}) => {
  const deterministicBaseline = buildNurseEvolutionNote(form);
  const authorizedFacts = buildAuthorizedNurseEvolutionFacts(form);
  ...
  const handleRefineWithAI = async () => {
    setIsRefining(true);
    try {
      const result = await refineNurseEvolutionNoteWithAI(form, authorizedFacts, deterministicBaseline);
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-16">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-teal-50 text-teal-800 border border-teal-200">
          ...
          Enfermeiro • Evolução de Enfermagem
        </span>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7 border border-slate-200/80 shadow-2xs space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-500 leading-relaxed">
          Revise a narrativa clínica estruturada gerada a partir dos dados preenchidos. Você pode refinar a coesão textual com IA ou copiar diretamente para o prontuário.
        </p>
      </div>
      ...
      {refinementStatus?.error && (
        <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>{refinementStatus.error}</span>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
        <div className="p-4 sm:p-5 bg-slate-50/80 border-b border-slate-200/80 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs sm:text-sm font-bold text-slate-800">
              Texto da Evolução de Enfermagem
            </span>
        ...
        <div className="px-5 py-3 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
          <span className="inline-flex items-center gap-1.5 text-emerald-700 font-medium">
            ...
            Trava de segurança clínica e de fatos autorizados ativa (8 travas de segurança)
          </span>
