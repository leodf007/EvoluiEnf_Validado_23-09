...
/**
 * PostGenerationVerifier for Nurse Surgical Clinic Evolution (NURSE_EVOLUTION_SURGICAL_CLINIC).
 ...
 * 3. DeviceLock: Devices and drains must strictly match authorized facts.
 ...
 * 10. SurgicalFactLock: Prohibits unrecorded surgery names, dates, anesthesia, complications, and drains.
 */
export function verifyNurseSurgicalEvolutionAIRefinedResponse(
  ...
  const surgicalLock = verifySurgicalFactLock(fullRefinedText, authorizedFacts, deterministicBaseline);
  ...
  if (!surgicalLock.passed) reasons.push(surgicalLock.message);
