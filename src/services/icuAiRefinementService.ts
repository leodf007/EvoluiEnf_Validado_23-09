...
import { verifyICUAIRefinedResponse } from '../engine/icuPostGenerationVerifier';
...
/**
 * Refines the ICU nursing note using server-side Gemini API (or strict deterministic fallback).
 * Evaluates PrivacyGate first, calls the server endpoint, and runs verifyICUAIRefinedResponse.
 */
export async function refineICUNoteWithAI(
  form: Partial<TechnicianICUNursingNoteForm>,
  ...
  try {
    ...
    // Step 3: PostGenerationVerifier for ICU
    const verification = verifyICUAIRefinedResponse(
      structuredResult,
      authorizedFacts,
      canonicalNarrative
    );
