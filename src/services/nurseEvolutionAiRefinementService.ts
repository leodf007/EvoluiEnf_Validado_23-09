...
import { verifyAIRefinedResponse, AIRefinedStructuredResponse } from '../engine/postGenerationVerifier';
import { AIRefinementRequestResult, DEFAULT_AI_MODEL } from './aiRefinementService';

/**
 * Refines the Nurse Evolution note using server-side Gemini API (or strict deterministic fallback).
 * Evaluates PrivacyGate first, calls the server endpoint, and runs PostGenerationVerifier
 * with NurseJudgmentLock, PhysicalExamFactLock, and ResponseToCareLock.
 */
export async function refineNurseEvolutionNoteWithAI(
  form: Partial<NurseEvolutionForm>,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        error: res.status === 429 ? message : 'Não foi possível realizar o refinamento automático neste momento. A evolução estruturada foi preservada.',
      };
    }
    ...
    if (!data.success || !data.result) {
      return {
        ...
        error: 'Não foi possível realizar o refinamento automático neste momento. A evolução estruturada foi preservada.',
      };
    }

    const structuredResult: AIRefinedStructuredResponse = data.result;

    // Step 3: PostGenerationVerifier for Nurse Evolution
    ...
    if (!verification.approved) {
      ...
      return {
        ...
        error: 'O refinamento automático foi descartado porque introduziu ou alterou informações não autorizadas. A evolução estruturada foi preservada.',
      };
    }
