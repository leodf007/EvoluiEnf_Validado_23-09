...
import { verifyNurseMedicalEvolutionAIRefinedResponse } from '../engine/nurseMedicalEvolutionPostGenerationVerifier';
import { AIRefinementRequestResult, DEFAULT_AI_MODEL } from './aiRefinementService';

export async function refineNurseMedicalEvolutionWithAI(
  form: Partial<NurseMedicalEvolutionForm>,
  ...
  try {
    ...
    // Step 3: PostGenerationVerifier validation
    const verification = verifyNurseMedicalEvolutionAIRefinedResponse(
      data,
      authorizedFacts,
      canonicalNarrative
    );
    ...
    if (verification.approved) {
      ...
      return {
        ...
        verificationDetails: verification,
        error: 'Texto refinado reprovado pelo PostGenerationVerifier de Clínica Médica.',
      };
    }
  } catch (err: any) {
    return {
      ...
      requestStatus: 'provider_error',
      verifierStatus: 'not_executed',
      error: err?.message || 'Falha de conexão com serviço de refinamento.',
    };
  }
}
